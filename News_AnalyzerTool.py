...
class News_AnalyzerTool:
    @tool("Content Loader")
    def extract_text_from_pdf(query: str):
        """Load and process the content of BABOK_Guide_v3_Member_2015.pdf, filtered by the search phrase if specified."""
        global_retriever_results = []
        directory_path = r"C:\Users\abdes\Desktop\devpy\crewai-updated-tutorial-hierarchical\assets"
        for filename in os.listdir(directory_path):
            file_path = os.path.join(directory_path, filename)

            if filename==('BABOK_Guide_v3_Member_2015.pdf'):
